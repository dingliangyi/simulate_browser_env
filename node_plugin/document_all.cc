...
void get_document_all(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
...
void Init(Local<Object> exports, Local<Object> module) {
  ...
  Local<Context> context = isolate->GetCurrentContext();
  Local<FunctionTemplate> method_template = FunctionTemplate::New(isolate, get_document_all);
  exports->Set(context, String::NewFromUtf8(isolate, "get_document_all").ToLocalChecked(), method_template->GetFunction(context).ToLocalChecked()).FromJust();
}
